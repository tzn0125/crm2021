...
<script type="text/javascript">
	...
	function changeStage(stage,i) {
		...
		$.ajax({
			...
			data : {
				"id" : "${tran.id}",
				"stage" : stage,
				"money" : "${tran.money}",	//生成交易历史所需
				"expectedDate" : "${tran.expectedDate}" //生成交易历史所需
			},
			type : "post",
			dataType : "json",
			success : function (data) {
				 ...
				 */
				if(data.success){
					//改变阶段成功后，需要在详细信息页进行局部刷新 刷新阶段，可能性，修改人，修改时间
	...
	function changeIcon(stage,index1) {
		...
		//正常和丢失阶段分界点
		//（前提：简单类型：八大数据类型 + String类型）  java对象转js对象  可以直接转换
		//尖括号和百分号 还必须在""
		var point = "<%=point%>";

		//如果当前阶段可能性为0,前七个一定是黑圈，之后的丢失阶段找出红叉和黑叉
		//在js中不用equals
		...
		//当前阶段不为0，丢失阶段一定是黑叉，再在正常阶段区分其他状态
		}else {
			//正常阶段
			for(var i=0;i<point;i++){
				//如果是当前阶段
...
<body>
	...
	<div style="position: relative; left: 40px; top: -50px;">
		...
		<%
			...
			//判断当前阶段
			//如果当前阶段的可能性为0，代表前面7个一定是黑圈，后面两个一个是红叉一个黑叉
			if("0".equals(currentPossibility)){
				//判断可能性为0的阶段哪个是红叉，那个是黑叉
				//先找出可能性为0的阶段
				for(int i=0;i<dvList.size();i++){
					DicValue dv = dvList.get(i);
					String stage = dv.getValue();
					String stagePossibility = pMap.get(stage);
					if("0".equals(stagePossibility)){
						//找到这些可能性为0的阶段中哪个阶段是现在的阶段,它就是红叉
		...
		<%
			...
			//不为0，后面两个一定是黑叉，前面7个有可能是绿圈，绿色标记，黑圈。
			}else {
				//准备当前阶段的下标
				int index = 0;
				for(int i=0;i<dvList.size();i++){
		...
		<%
					...
					//遍历出来的可能性不为0，代表他们是正常阶段
					}else {
						//在他们中继续筛选
						//
						//如果当前阶段的i等于index,他就是当前正常阶段
						if(i == index){
							//绿色标记
